...
# TODO: Test scenario where there are two hosts with same name in different zones
# GET /hosts/{host}
...
sub get_host ($self) {
  ...
  unless ($fqdn && $fqdn =~ /\./) {
    return $self->render(
      openapi => { error => 'Bad Request', message => "Invalid FQDN format: '$fqdn'" },
      status  => 400
    );
  }
  ...
  if ($host_id <= 0) {
    return $self->render(
      openapi => { error => 'Not Found', message => "Host '$fqdn' not found" },
      status  => 404
    );
  }
...
sub add_host ($self) {
  ...
  my ($zone_id, $domain);
  for my $i (1 .. $#parts) {
    my $zone_name = join('.', @parts[$i .. $#parts]);
    my $host_part = join('.', @parts[0 .. $i - 1]);
    $zone_id = Sauron::BackEnd::get_zone_id_by_name($zone_name);
    if ($zone_id > 0) {
      $domain = $host_part;
      last;
    }
  ...
  unless ($zone_id > 0) {
    return $self->render(
      openapi => { error => 'Not Found', message => "No zone found for FQDN '$fqdn'" },
      status  => 404
    );
  }

  my $existing_id = Sauron::BackEnd::get_host_id($zone_id, $domain);
  if ($existing_id > 0) {
    return $self->render(
      openapi => { error => 'Conflict', message => "Host '$fqdn' already exists (id=$existing_id)" },
      status  => 409
    );
  }
