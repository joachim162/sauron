...
mkdir -p /srv/sauron/logs /usr/local/sauron/logs
chown www-data:www-data /srv/sauron/logs /usr/local/sauron/logs
...
chmod +x /srv/sauron/cgi/sauron.cgi /srv/sauron/cgi/browser.cgi 2>/dev/null || true

echo "=== Starting Sauron (Apache CGI) ==="
exec "$@"
